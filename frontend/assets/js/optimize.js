document.addEventListener('DOMContentLoaded', ()=>{
  ...
  btn.addEventListener('click', ()=>{
    ...
    const moist = Number(document.getElementById('moist').value); // ✅ added
    ...
    out.innerHTML = '<h3>Result</h3>' + recs.map(r=>`<div class="card"><p class="small">${r}</p></div>`).join('') +
      `<hr/><p><b>Revenue/ha:</b> ₹${rev} <b>Profit/ha:</b> ₹${profit}</p>`;
    ...
    if (ctx){
      ...
      window._profitChart = new Chart(ctx, {
        ...
        data:{
          ...
          datasets:[{label:'₹', data:[rev, cost, profit]}]
        }
      });
    }
  });
