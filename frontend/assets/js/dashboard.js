document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  const API = 'https://cropiq1.onrender.com';  // ✅ No trailing slash
  ...
  async function loadDashboard(){
    try{
      ...
      window.barChart = new Chart(ctx, {
        type:'bar',
        data:{labels:labels, datasets:[{label:'t/ha', data:vals}]},
      ...
      window.pieChart = new Chart(ctx2, {
        type:'pie',
        data:{labels:rlabels, datasets:[{data:rvals}]},
        options:{responsive:true}
      });

      // alerts
      const alertsBody = $('alertsBody');
      alertsBody.innerHTML = data.alerts.length
        ? data.alerts.map(a=>`<tr><td>${a.date}</td><td>${a.loc}</td><td>${a.type}</td><td>${a.msg}</td></tr>`).join('')
  ...
  async function loadHistory(){
    try{
      const res = await fetch(`${API}/history?top=6`);
      const data = await res.json();
